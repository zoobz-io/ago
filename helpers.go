...
func Gate[T any](name string, predicate func(context.Context, *Flow[T]) bool) pipz.Processor[*Flow[T]] {
	return pipz.Apply(pipz.Name(name), func(ctx context.Context, f *Flow[T]) (*Flow[T], error) {
		if predicate(ctx, f) {
			return f, nil
		}
		return f, nil
...
func RateLimiter[T any](name string, requestsPerSecond float64, burst int) *pipz.RateLimiter[*Flow[T]] {
	return pipz.NewRateLimiter[*Flow[T]](pipz.Name(name), requestsPerSecond, burst)
}
